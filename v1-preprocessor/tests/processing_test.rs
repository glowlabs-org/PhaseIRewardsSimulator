...
fn get_test_v1_history() -> V1History {
    V1History {
        ...
        solar_farms: vec![V1SolarFarm {
            farm_id: "45-ab".to_string(),
            first_rewards_week: 34,
...
fn happy_path_test() {
    ...
    assert_eq!(farm.net_weekly_carbon_credits, "120000");
...
#[test]
fn test_duplicate_farm_id() {
    let mut v1_history = get_test_v1_history();
    v1_history.solar_farms.push(V1SolarFarm {
        farm_id: "45-ab".to_string(),
        first_rewards_week: 50,
...
fn test_invalid_glow_split_sum() {
    let mut v1_history = get_test_v1_history();
    v1_history.solar_farms[0].reward_splits[0].glow_split_percent_6_decimals = "999999".to_string();
    let result = process_v1_history(v1_history);
    assert!(result.is_err());
}
...
fn test_invalid_deposit_split_sum() {
    let mut v1_history = get_test_v1_history();
    v1_history.solar_farms[0].reward_splits[0].deposit_split_percent_6_decimals = "1".to_string();
    let result = process_v1_history(v1_history);
    assert!(result.is_err());
}
