...
fn build_full_weeks_map() -> HashMap<String, String> {
    ...
    for w in 96u64..=287u64 {
        // Provide a baseline value; week 96 can be overwritten by tests if needed.
...
fn test_negative_exactly_minus_ten_pruned() {
    // Start with 10 at week 96, apply a deposit with ceil/usdg of 20 => leftover becomes -10.
    ...
    usdg_per_week.insert("96".to_string(), "10".to_string());
...
fn test_negative_below_minus_ten_errors() {
    // Start with 10 at week 96, apply a deposit causing deduction 21 => leftover becomes -11, should error.
    let mut usdg_per_week = build_full_weeks_map();
    usdg_per_week.insert("96".to_string(), "10".to_string());
    let (solar_farms, migrating_to_utah) = basic_farm();
    let v1_history = V1History {
